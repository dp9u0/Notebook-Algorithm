...
describe('SplayTree', function () {
  ...
  describe('#search()', function () {
    it(`search should work ok(root left)`, function () {
      const input = [8, 4, 12, 2, 6, 10, 14, 1, 3, 5, 7, 9, 11, 13, 15];
      let tree = Tree.fromArray(input);

      const element = 4;
      expect(tree.search(element), `tree should contains ${element}`).to.be.true;
      expect(tree.root.value, `after search ${element},this value should be splay to the root`).to.equal(element);

    });
    it(`search should work ok(root right)`, function () {
      const input = [8, 4, 12, 2, 6, 10, 14, 1, 3, 5, 7, 9, 11, 13, 15];
      let tree = Tree.fromArray(input);
    ...
    it(`search should work ok(root left left)`, function () {
      const input = [8, 4, 12, 2, 6, 10, 14, 1, 3, 5, 7, 9, 11, 13, 15];
      let tree = Tree.fromArray(input);
      const element = 2;
      expect(tree.search(element), `tree should contains ${element}`).to.be.true;
    ...
    it(`search should work ok(root left right)`, function () {
      const input = [8, 4, 12, 2, 6, 10, 14, 1, 3, 5, 7, 9, 11, 13, 15];
      let tree = Tree.fromArray(input);
      const element = 6;
      expect(tree.search(element), `tree should contains ${element}`).to.be.true;
    ...
    it(`search should work ok(root right left)`, function () {
      const input = [8, 4, 12, 2, 6, 10, 14, 1, 3, 5, 7, 9, 11, 13, 15];
      let tree = Tree.fromArray(input);
      const element = 10;
      expect(tree.search(element), `tree should contains ${element}`).to.be.true;
    ...
    it(`search should work ok(root right right)`, function () {
      const input = [8, 4, 12, 2, 6, 10, 14, 1, 3, 5, 7, 9, 11, 13, 15];
      let tree = Tree.fromArray(input);
      const element = 14;
      expect(tree.search(element), `tree should contains ${element}`).to.be.true;
    ...
    it(`randomTest should work ok()`, function (done) {
      ...
      for (let j = 0; j < INPUT_COUNT; j++) {
        ...
        if (op === 1) {
          ...
          expect(search, `search result should be ${set.has(value)}`).to.equal(set.has(value));
          if (search) {
            expect(tree.root.value, `after search ${value},this value should be splay to the root`).to.equal(value);
          }
        } else {
          tree.insert(value);
          set.add(value);
