...
下面讲到的三种特殊的排序方式是在某些特殊情况下实现的线性排序:
...
通过一次遍历记录每个数字出现的次数,然后通过遍历{键-次数}数据对输出排序结果.

[计数排序实现](../src/sort/countingSort.js)

### 基数排序

基数排序也是利用了桶的概念: 将整数按每个位上的数字分组,然后再进行排序,排序完成后再按照更高位进行分组.
...
桶排序或所谓的箱排序是一个排序算法,工作的原理是将数组分到有限数量的桶里.每个桶再个别排序(有可能再使用别的排序算法或是以递归方式继续使用桶排序进行排序)

步骤为:

1. 设置一个定量的数组当作空桶
2. 寻访序列,并且把项目一个一个放到对应的桶
3. 对每个不是空的桶进行排序
